...
def read_world():
    with open(FILENAME_WORLD, 'r') as p:
        lines = filter(None, map(str.strip, p.readlines()))

    convert = {'.' : 'wall', 'r' : 'road', 's' : 'start', 'e' : 'end'}
    
    w, h = len(lines[0]), len(lines)
    world = [[convert[lines[i][j]] for j in xrange(w)]
             for i in xrange(h)]
    
    return world

def pad_string(sentence, length):
    length_front = (length - len(sentence))/2
...
def generate_binaries(length, num_samples):
    ...
    for i in xrange(length):
        ...
        samples.append(map(int, bin(num)[2:].zfill(length)))
...
class MainApp(tk.Frame, object):
    ...
    def callback_evolve(self):
        ...
        self.plans = map(self.mutate_plan, childs)
    ...
    def get_score(self, plan):
        ...
        cell_walked = [[False for j in xrange(self.dim_x)]
                       for i in xrange(self.dim_y)]
    ...
    def select_plans(self, plans, num_samples): 
        scores = map(self.get_score, plans)
    ...
    def run(self):
        self.start_trip(map(int, '110110001100011111'), 0.01)
